        ...
        <div class="row">
            <div class="col-md-12">
                    <div v-if="authStatus" class="d-flex flex-row add-comment-section mt-4 mb-4">
                        ...
                        <input @keyup.enter="addComment({comment: content, article_id: postId, creator_id: auth.id})" type="text" class="form-control mr-3" v-model="content" placeholder="Добавить Коментарий">
                        <button @click="addComment({comment: content, article_id: postId})" class="btn btn-primary" type="button">Добавть</button>
                    </div>
                    <div class="card-body">
                    ...
                    <a href="#" class="btn btn-primary btn-sm btn-block" role="button"><span class="glyphicon glyphicon-refresh"></span> Загрузить еще</a>
            </div>
        </div>
    </div>
</template>
